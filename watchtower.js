...
var newid = registry.push({'repo':req.body.repo,'secret':crypto.createHash('sha1').update(req.body.secret).digest('hex'),'connection':'http','coninfo':req.body.url});
res.send("success");
});
...
case "register":
  ...
  var newid = registry.push({'repo':data.name,'secret':crypto.createHash('sha1').update(data.secret).digest('hex'),'connection':'ws','coninfo':ws.id});
  ws.send({type:'success',id:newid});
  console.log("Listener watching for " + data.name);
  break;
case "deregister":
  if(data.id<0||data.id>=registry.length) {
    ws.send(JSON.stringify({'type':'error','error':'bad id '+data.id}));
  } else if(registry[data.id].coninfo==ws.id) {
    registry.splice(data.id,1);
    ws.send(JSON.stringify({type:'deregister'}))
  } else {
    ws.send(JSON.stringify({'type':'error','error':'bad id '+data.id}));
  }
  break;
default:
  ws.send(JSON.stringify({'type':'error','error':'unknown operation '+data.type}));
}
});
});
...
client.send({type:'forward',body:payload,headers:headers});
